...
The coverage curves in \Cref{fig:coverage_comparison} show that the proposed hierarchical GVD model increases map coverage faster than the baselines on all three maps. It also reaches a higher final overall coverage than the frontier, graph-guided frontier, and prior-topology greedy exploration baselines. Notably, this advantage remains even against prior-topology greedy exploration, which is given the green prior graph shown in \Cref{fig:test_maps}.

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\linewidth]{imgs/slam_rooms_trajectory_comparison.pdf}
